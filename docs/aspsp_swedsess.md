...
|![](https://img.shields.io/badge/status-important-important.svg)| Consent | **Decoupled Only** `scaStatus` will transition through the following statuses, and can at any time end up in `failed`: <br> `received` -> <br> `authenticationStarted` (with `AUTO_START_TOKEN`) -> <br> `started` (with `AUTO_START_TOKEN`) -> <br> `finalised` |
|![](https://img.shields.io/badge/status-important-important.svg)| PIS | **Decoupled Only** `scaStatus` will transition through the following statuses, and can at any time end up in `failed`: <br> `received` -> <br> `authenticationStarted` (with `AUTO_START_TOKEN`) -> <br> (`Conditional`) `authoriseCreditorAccountStarted` (with `AUTO_START_TOKEN`) -> <br> `started` (with `AUTO_START_TOKEN`) -> <br> `finalised` |
