...
`combinedServiceIndicator` if true this indicates that the session will be used to initiate a payment also. It has no practical meaning at this time.
...
The list of links can be used for further actions on the consent. The `self` one is obviously to retrieve the created consent request. The `status` is for accessing the status of the request. And `startAuthorisation`is the next step in the sequence of calls that is needed to get a consent.
